...
// MAKE SURE TO RUN DATAGEN AFTER CHANGING ANYTHING HERE

public class EVConfiguredFeatures {

    private static TreeConfiguration.TreeConfigurationBuilder createStraightBlobTree(BlockState logBlock, Block leavesBlock, int baseHeight, int heightRandA, int heightRandB, int radius) {
    ...
    private static TreeConfiguration.TreeConfigurationBuilder createStraightRubber() {
        return createStraightBlobTree(GTBlocks.RUBBER_LOG.get().changeNatural(GTBlocks.RUBBER_LOG.getDefaultState(), true), GTBlocks.RUBBER_LEAVES.get(), 5, 2, 0, 2).ignoreVines();
    }
